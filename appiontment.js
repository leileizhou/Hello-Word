define(function(){
  ...
  //总预约时间
	...
	//画布中园的圆心
	var canvas_center = {x:document.body.clientWidth/2,y:document.body.clientWidth/2};
  ...
  var ctx = canvas_kedu.getContext("2d");
	...
	appiontmentControl['drawKeDu00'] = function(){
 	...
 	appiontmentControl['drawKeDu'] = function(flag,from,to){
 		ctx.clearRect(0,0,1000,1000);
		if(flag){
			if(!to){
				to = from + defaultJiaoDuCha;
			}else{
				to = Math.max(from + defaultJiaoDuCha,to);
			}
			to = Math.max(from + defaultJiaoDuCha,to);
		}else{
			from = zongRotate;
	...
	//优化
	appiontmentControl['drawKeDu11'] = function(flag,from,to){
 		ctx.clearRect(0,0,1000,1000);
		if(flag){
			if(!to){
				to = from + defaultJiaoDuCha;
 	...
 	var startCenter = {x:point_x,y:point_y};
	...
	appiontmentControl['drag1'] = function(element){
		...
		element.addEventListener('touchend',function(){
 		...
 		mc1.on('panleft panright panup pandown',function(evt){
			...
			// if(evt.direction == 4 && zongEndRotate >= 450){
			// 	return;
			// }
			...
			appiontmentControl['drawKeDu11'](true,currentStartEleDu,zongEndRotate);
	...
	appiontmentControl['drag2'] = function(element){
 		...
 		mc1.on('panleft panright panup pandown',function(evt){
			...
			if(zongEndRotate >350 && zongEndRotate<360 && evt.direction == 4){
				...
				appiontmentControl['drawKeDu11'](false,0,360);
			}
			...
			if(zongEndRotate > currentStartEleDu+360){
				...
				appiontmentControl['drawKeDu11'](false,zongRotate,zongRotate+360);
				return;
			}
			...
			appiontmentControl['drawKeDu11'](false,'',zongEndRotate);
	...
	function resetArrs(){
 	...
 	appiontmentControl['init'] = function(){
		...
		this['drawKeDu00']();
 		this['drawKeDu'](0.25,0,defaultJiaoDuCha);
		initKeDuPan();
